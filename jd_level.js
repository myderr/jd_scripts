...
京东等级
更新时间：2021-6-26
creditLevel，最高11级，低于8的抢啥都很难，低于5的就不要抢购了。
==========================Quantumultx=========================
[task_local]
#jd等级
...
!(async () => {
  ...
  //错误统一通知  领取 未种植 错误
  if ($.isNode() && error_msg) {
    await notify.sendNotify(`【${$.name}】以下用户有需要处理\n`, `${error_msg}`)
  }
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            $.userScore = data?.model?.scoreDescription?.userScore;
          } else {
            $.log('京东服务器返回空数据');
          }
        }

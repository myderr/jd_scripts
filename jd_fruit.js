...
let message = '', subTitle = '', option = {}, isFruitFinished = false, error_msg = '';
...
!(async () => {
  ...
  let ck_failure_title = `【提示】以下用户cookie已失效\n`;
  let ck_failure_mag = ``;
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        ...
        if ($.isNode()) {
          ck_failure_mag += `京东账号${$.index} ${$.UserName}\n\n`
  ...
  //ck失效统一通知
  if ($.isNode() && ck_failure_mag) {
    await notify.sendNotify(`${ck_failure_title}`, `${ck_failure_mag}`)
  }
  //错误统一通知  领取 未种植 错误
  if ($.isNode() && error_msg) {
    await notify.sendNotify(`【京东农场】以下用户有需要处理\n`, `${error_msg}`)
  }
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        if ($.isNode()) {
          error_msg += `【京东账号${$.index}】${$.nickName || $.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n\n`;
